...
class Qualification extends \yii\db\ActiveRecord
{
    ...
    public static function getQualifications($withoutId = true){
        if($withoutId){
            return \yii\helpers\ArrayHelper::map(self::find()->where(['is_deleted'=>0, 'status'=>1])->all(), 'name', 'name');
        }else{
            return \yii\helpers\ArrayHelper::map(self::find()->where(['is_deleted'=>0, 'status'=>1])->all(), 'id', 'name');
        }
    }
}
