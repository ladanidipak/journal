...
<hr>
<?php $form = ActiveForm::begin([ 'options'=>['enctype'=>'multipart/form-data'],'id'=>'sent-request-form','enableAjaxValidation' => false, 'enableClientValidation' => true, 'fieldConfig' => ['options' => ['class' => 'col-sm-12'], 'checkboxTemplate' => "<label class=\"control-label\">{labelTitle}</label>\n<div>\n{beginLabel}\n{input}\n{endLabel}\n{error}\n{hint}\n</div>"]]); ?>
<div class="row">
    <?= Html::dropDownList('branches',null,backend\models\Branch::getBrancheNames(), ['id'=>'search-branch-pop-2','class'=>'chosen-select-width','multiple'=>'multiple', 'data-placeholder'=>'Select Branch']) ?>
</div>
<br>
    <div class="row">
...
<?php
$this->registerJs(
    "var table_search_2 = $('#article-review-grid table').DataTable({\"aLengthMenu\": [ 10, 25, 50, 100, 200 ]});
    $('#search-branch-pop-2').chosen({width : '250px'});
        $('#search-branch-pop-2').change(function(){
            if($(this).val() != null){
                var search = $('#search-branch-pop-2').val().join('|');
                table_search_2.column( 3 ).search( search, true, false ).draw();
            }else{
                table_search_2.column( 3 ).search( '', true, false ).draw();
            }
        });
    ",
    View::POS_READY
